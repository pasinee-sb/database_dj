...
class NewSongForPlaylistForm(ModelForm):
    ...
    def __init__(self, playlist_id):
        """called when an instance of the class is created. Takes playlist_id as an argument to identify the playlist that 
        the user wants to add a song to"""
        super().__init__()
        """calls the __init__ method of the parent class (ModelForm) and passes the additional arguments and keyword arguments to it. 
        to ensure that the form is properly initialized with all of the fields and validators defined in the Meta class."""
        self.song.choices = [
            (song.id, f"{song.title} - {song.artist}") for song in Song.query.all()]
        """sets the choices attribute of the song field to a list of tuples,
          that each contains ID as the first element and the title and artist as the second element. 
        ...
        """stores playlist_id as an instance attribute of the form. 
        so the validate method can access the ID of the playlist that the user wants to add a song to."""

    def validate(self, extra_validators=None):
        """The validate method is called by the validate_on_submit method of the form when the user submits the form. 
        for additional validation on the data submitted by the user."""

        if not super().validate():
            """calls the validate method of the parent class (ModelForm) to perform the basic validation on the form data. 
            If the parent validation fails, the method immediately returns False and 
            does not proceed with the additional validation."""
            return False

        # run additional validators if they were passed in(not sure why and how, but fixed the bug)
        if extra_validators:
            for validator in extra_validators:
                validator(self)

        song = Song.query.get(self.song.data)
        """retrieves the Song object that is corresponding to the song ID selected by the user in the song field."""
